...
export function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex lg:w-[280px] xl:w-[320px] flex-shrink-0 flex-col rounded-lg bg-bg-surface overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        ...
        {session && (
          <Link
            href="/share"
            className="flex h-7 w-7 items-center justify-center rounded-full text-text-secondary transition-colors hover:bg-menu-hover hover:text-text-primary"
        ...
        <div className="flex gap-1.5 px-4 py-2">
          {(["all", "playlists", "coders"] as const).map((tab) => (
            <button
              ...
              className={
                filter === tab
                  ? "rounded-full bg-login-bg px-3 py-1 text-[11px] font-bold text-login-text"
                  : "rounded-full bg-bg-card px-3 py-1 text-[11px] font-bold text-text-secondary hover:bg-bg-card-hover hover:text-text-primary transition-colors"
              }
            >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 scrollbar-thin">
        {!session ? (
          <div className="space-y-1 px-1 py-2">
            <Link
              href="/explore"
              className="flex items-center gap-3 rounded-md p-2.5 transition-colors hover:bg-menu-hover"
            >
              <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded bg-bg-card">
          ...
          <div className="space-y-1 px-1">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex items-center gap-3 rounded-md p-2 animate-pulse">
                <div className="h-10 w-10 rounded bg-bg-card" />
            ...
            {items.map((item) =>
              item.type === "playlist" ? (
                <Link
                  ...
                  className="flex items-center gap-3 rounded-md p-2 transition-colors hover:bg-menu-hover"
                >
                  <div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded bg-bg-card">
                ...
                <Link
                  ...
                  className="flex items-center gap-3 rounded-md p-2 transition-colors hover:bg-menu-hover"
                >
                  <div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-full bg-bg-card">
